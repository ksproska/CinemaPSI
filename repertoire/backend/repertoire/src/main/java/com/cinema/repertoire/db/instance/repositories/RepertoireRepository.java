...
public interface RepertoireRepository extends JpaRepository<Repertoire, Long> {
    ...
    @Query(value = "SELECT new com.cinema.repertoire.web.dtos.RepertoireDTO(r.starting, r.ending, h.hallNumber, r.movieVersionId) FROM Repertoire r LEFT JOIN Hall h ON r.hallId = h.id WHERE r.starting >= NOW() AND r.starting < :end " +
            "AND h.cinemaId = :cinemaId")
    List<RepertoireDTO> findRepertoireDtoByStartingAndCinemaId(Long cinemaId, LocalDateTime end);
}
